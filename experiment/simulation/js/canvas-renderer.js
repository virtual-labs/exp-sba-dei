 ...
 */
    drawNF(nf) {
        ...
        // Draw HTTP Protocol Badge (Optional)
        if (nf.config && nf.config.httpProtocol) {
            const protocol = nf.config.httpProtocol;
            const badgeWidth = 30;
            ...
            // Badge background
            this.ctx.fillStyle = protocol === 'HTTP/2' ? '#3498db' : '#95a5a6';
            ...
            this.ctx.fillText(protocol, badgeX + badgeWidth / 2, badgeY + badgeHeight / 2);
        }

        // Status indicator - red for STARTING, green for STABLE
        let statusColor;
        if (nf.status === 'STABLE') {
            statusColor = '#4caf50'; // Green for stable
        } else if (nf.status === 'STARTING') {
            statusColor = '#e74c3c'; // Red for starting
        } else {
            statusColor = '#f39c12'; // Orange for other states
        }
        
        this.ctx.fillStyle = statusColor;
        this.ctx.beginPath();
        this.ctx.arc(x + width - 6, y + height - 6, 4, 0, Math.PI * 2); // Slightly larger dot
        this.ctx.fill();
        
        // Add a white border around the status dot for better visibility
    ...
    /**
     * Draw a connection between two NFs
    ...
    drawConnection(conn) {
        ...
        // Interface labels removed - only show in logs
    }
    ...
    handleCanvasClick(e) {
        ...
        if (clickedNF) {
            ...
            if (clickedBus) {
                console.log('✅ Clicked on Bus: (no configuration available)');
                // Buses are not configurable - just log the click
                // No config panel will be shown for buses
            } else {
    ...
    handleMouseMove(e) {
        ...
        if (this.isDragging) {
            if (this.draggedNF) {
                ...
                if (window.serviceManager) {
                    window.serviceManager.moveService(this.draggedNF.id, { x: newX, y: newY });
                } else if (window.nfManager) {
                    // Fallback for any remaining NF functionality
                    window.nfManager.moveNF(this.draggedNF.id, { x: newX, y: newY });
                }

                // Re-render canvas to show new position
                this.render();
                return;
            } else if (this.draggedBus) {
                const newX = x - this.dragOffset.x;
                const newY = y - this.dragOffset.y;
        ...
        // Check bus hover if no NF is hovered
        if (!hoveredNF) {
            for (const bus of allBuses) {
                if (this.isPointInBus(x, y, bus)) {
                    hoveredBus = bus;
    ...
    handleMouseDown(e) {
        ...
        // Check for Bus dragging if no NF was clicked
        if (!this.isDragging) {
            for (const bus of allBuses) {
                if (this.isPointInBus(x, y, bus)) {
                    this.isDragging = true;
    ...
    /**
     * Get Bus at specific position
     * @param {number} x - X coordinate
     * @param {number} y - Y coordinate
     * @returns {Object|null} Bus at position or null
     */
    ...
    /**
     * Check if point is inside Bus line
     * @param {number} x - X coordinate
     * @param {number} y - Y coordinate
    ...
    isPointInBus(x, y, bus) {
        const tolerance = 15; // Click tolerance for bus lines

        if (bus.orientation === 'horizontal') {
            return x >= bus.position.x &&
                x <= bus.position.x + bus.length &&
                y >= bus.position.y - tolerance &&
                y <= bus.position.y + tolerance;
        } else {
            return x >= bus.position.x - tolerance &&
                x <= bus.position.x + tolerance &&
                y >= bus.position.y &&
 ...
 */
    drawBus(bus) {
        ...
        // Highlight if hovered
        if (this.hoveredBus === bus.id) {
            this.ctx.lineWidth = thickness + 4; // Make it even thicker when hovered
            this.ctx.strokeStyle = '#f39c12'; // Orange highlight
        }
        ...
        // No bus labels - buses are unnamed
